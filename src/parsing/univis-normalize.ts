...
import { UnivisExam, UnivisSchedule } from '../types/univis.js'
...
export function getCategoryPriority(categoryTitle: string): number {
  if (categoryTitle === THEORETICAL_CATEGORY) return 0
  if (categoryTitle === ELECTIVE_CATEGORY) return 1
  return 99
}

export function parseTitleCode(value: string) {
...
export function isExerciseTitle(value: string): boolean {
  return /^(Exercise:|Practical Exercise:|Übung zu:|Tutorial:)/i.test(
    stripTags(value),
  )
}

export function mapKindToCategory(kindLabel: string | null | undefined): string {
...
export function getExamDisplayLabel(label: string): string {
...
export function normalizeLectureLanguage(value: string | null | undefined): string | null {
...
export function extractLectureLanguage(detailLines: string[]): string | null {
...
export function normalizeScheduleLine(rawLine: string): UnivisSchedule[] {
...
export function normalizeDateRangeLine(rawLine: string) {
...
export function extractDefinitionContent(html: string, label: string): string | null {
...
export function combineDefinitionSections(html: string, labels: string[], sourceBaseUrl: string | null = null): string | null {
...
export function parseExamLines(detailLines: string[]): UnivisExam[] {
  return detailLines
    .map(line =>
      line.match(
...
export function parseAssignedLectureEntries(
  blockHtml: string,
  fallbackDateRange: { startDate: string | null; endDate: string | null } | null,
  sourceBaseUrl: string = 'https://univis.uni-kiel.de',
) {
